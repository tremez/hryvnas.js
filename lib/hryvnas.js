(function() {
  ...
  var words = [
    [
      ...
      'сім', 'вісім', 'девя\'ть', 'десять', 'одинадцать',
      'дванадцать', 'тринадцать', 'чотирнадцать', 'п\'ятнадцать',
      'шістнадцать', 'сімнадцать', 'вісімнадцать', 'дев\'ятнадцать'
    ],
    [
      '', '', 'двадцать', 'тридцать', 'сорок', 'п\'ятдесят',
  ...
  var hryvnas = ['гривня', 'гривні', 'гривень'];
  ...
  var parseNumber = function(number, count, currCode) {
    ...
    if (count === 0) {
      switch (currCode) {
        case 'UAH':
        default: {
          numeral = numeral + plural(number, hryvnas);
        }
      }
    } else if (count === 1) {
      if (numeral !== '  ') {
        numeral = numeral + plural(number, ['тисяча ', 'тисячі ', 'тисяч ']);
        //numeral = numeral.replace('один ', 'одна ').replace('дві ', 'дві ');
      }
    } else if (count === 2) {
      if (numeral !== '  ') {
        numeral = numeral + plural(number, ['мільйон ', 'мільйона ', 'мільйонів ']);
      }
    } else if (count === 3) {
      numeral = numeral + plural(number, ['мільярд ', 'мільярда ', 'мільярдів ']);
    }
