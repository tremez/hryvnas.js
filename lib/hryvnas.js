(function() {
  ...
  var hrivnas = ['гривня', 'гривні', 'гривень'];
  ...
  var parseNumber = function(number, count, currCode) {
    ...
    if (count === 0) {
      switch (currCode) {
        case 'UAH':
        default: {
          numeral = numeral + plural(number, hrivnas);
        }
      }
    } else if (count === 1) {
      if (numeral !== '  ') {
      ...
      if (numeral !== '  ') {
        ...
        numeral = numeral.replace('одна ', 'один ').replace('дві ', 'два ');
  ...
  var hryvnas = function(number, currCode) {
    if (!number) {
      return false;
    }
